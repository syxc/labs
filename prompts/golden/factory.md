...
- Commit only when asked. Disable `--no-verify` and `--no-gpg-sign` on commit. Review the staged
  ...
  `.gitignore` only within the authorized scope. Run the repo's configured hooks; keep the existing
  git author; do not add Co-Authored-By to commits. Title: English `<scope>: <summary>`, scope
